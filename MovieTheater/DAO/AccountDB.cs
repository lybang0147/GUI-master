...
namespace MovieTheater.DAO
{
    class AccountDB
    {
        ...
        public static bool UpdateAccountRole(string username, int accountType)
        {
            int result = myDB.ExecuteNonQuery("EXEC rolechange @username , @loaiTK ", new object[] { username, accountType });
            return result > 0;
        }
        ...
        public static bool InsertAccountSQL(string username,string password, int accountype)
        {
            int result = myDB.ExecuteNonQuery("EXEC themtaikhoan @username , @loaiTK , @idNV ", new object[] { username, password, accountype });
            return result > 0;
        }
